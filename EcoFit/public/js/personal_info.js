  ...
  imageUpload.addEventListener('change', (e) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        localStorage.setItem('userAvatar_' + currentUserData.profile_id, event.target.result);
      };
...
async function loadUserProfile() {
  ...
  if (loginInfoStr) {
    try {
      ...
      userId = userId || loginInfo.userId || loginInfo.profile_id || loginInfo.id;
      userEmail = userEmail || loginInfo.email;
      
      console.log('After parsing login_infor - userId:', userId, 'email:', userEmail);
    } catch (e) {
  ...
  try {
    ...
    if (userId) {
      userProfile = data.profile.find(p => p.profile_id === userId || p.profile_id === userId.toString());
    ...
    if (!userProfile) {
      ...
      console.log('Available profile_ids:', data.profile.map(p => p.profile_id));
...
function populateForm(userProfile) {
  ...
  try {
    ...
    if (dobInput && userProfile.dob) {
      ...
      if (dobValue) {
        if (dobValue.match(/^\d{4}-\d{2}-\d{2}$/)) {
          ...
          if (parts.length === 3) {
            dobInput.value = `${parts[2]}-${parts[1].padStart(2, '0')}-${parts[0].padStart(2, '0')}`;
          }
        }
    ...
    if (profileImage) {
      const savedAvatar = localStorage.getItem('userAvatar_' + userProfile.profile_id);
...
if (dobInput) {
  dobInput.addEventListener('blur', (e) => {
    ...
    if (value && !value.includes('-') && value.includes('/')) {
      ...
      if (parts.length === 3) {
        e.target.value = `${parts[2]}-${parts[1].padStart(2, '0')}-${parts[0].padStart(2, '0')}`;
      }
    }
  });
}
